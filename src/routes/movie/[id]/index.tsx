...
export const onGet: RequestHandler<MovieDetailResponseType> = async ({
  params,
}) => {
  ...
  const similarMovies =
    (await APIRequest.getSimilarMovies(params.id)) ?? undefined
                  ...
                  <div class='flex flex-col'>
                    ...
                    <div class='mt-8'>
                      {movieDetails?.tagline && (
                        <h3 class='font-thin italic text-lg text-gray-400'>
                          {movieDetails?.tagline}
                        </h3>
                      )}
...
export const head: DocumentHead<typeof onGet> = ({
  data: { movieDetails },
}) => {
  return {
    title: `${movieDetails?.title} (${getYear(movieDetails?.release_date)}) - ${
      movieDetails?.tagline
    }`,
    meta: [
      {
